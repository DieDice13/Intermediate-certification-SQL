...
CREATE PROCEDURE MoveUserToOldTable(IN userId INT)
BEGIN
    -- Переменная для хранения количества пользователей
    DECLARE userCount INT;
    
    -- Подсчитываем количество пользователей с указанным ID
    SELECT COUNT(*) INTO userCount FROM users WHERE id = userId;
    
    -- Если пользователь существует, перемещаем его в таблицу users_old и удаляем из users
    IF userCount > 0 THEN
    ...
    ELSE
        -- Откатываем транзакцию, если пользователь не найден
        ROLLBACK;
        SELECT 'Пользователь с указанным ID не найден';
    END IF;
END //
...
-- Вызов процедуры
CALL MoveUserToOldTable(6); -- В скобках указываем id пользователя которого хотим переместить

-- Проверяем переместились ли пользователи
...
BEGIN
    DECLARE current_hour INT; -- Текущий час
    DECLARE greeting VARCHAR(50); -- Приветствие
...
-- Вызов функции которая должна вернуть приветсвие в зависимости от времени суток
SELECT hello();
...
/*
Триггеры - хранимые процедуры, которые автоматически вызываются (или "срабатывают") при выполнении определенного события в базе данных.
Эти события могут быть разными, например, вставка (INSERT), обновление (UPDATE) или удаление (DELETE) данных из таблицы.
*/
